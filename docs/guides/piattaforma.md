...
| **Amministrazione** | Gestisce anagrafiche, candidature degli insegnanti, configurazione dei prezzi e delle promozioni, territori delle scuole esterne, verifica dei pagamenti e reportistica. Endpoint: [Admin - Users](/api/admin-users), [Admin - Lessons](/api/admin-lessons), [Admin - Availability](/api/admin-availability), [Admin - Finance](/api/admin-finance), [Admin - Price Changes](/api/admin-price-changes), [Admin - Settlements](/api/admin-settlements), [Admin - Job Runs](/api/admin-job-runs), [Admin - External Schools](/api/admin-external-schools), [Admin - Monitoring](/api/admin-monitoring). |
